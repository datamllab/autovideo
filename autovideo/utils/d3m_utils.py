...
def build_pipeline(config):
    ...
    alg_python_path = 'd3m.primitives.autovideo.augmentation.channel_shuffle'
    ...
    step_7.add_argument(name='outputs', argument_type=ArgumentType.CONTAINER, data_reference=f'steps.{step_4.index}.produce')
    # Add hyperparameters
    for key, value in config.items():
        step_7.add_hyperparameter(name=key, argument_type=ArgumentType.VALUE, data=value)
    step_7.add_output('produce')
    pipeline_description.add_step(step_7)

    #Step 8: Construct the predictions
    step_8 = PrimitiveStep(primitive=index.get_primitive('d3m.primitives.autovideo.common.construct_predictions'))
    step_8.add_argument(name='inputs', argument_type=ArgumentType.CONTAINER, data_reference=f'steps.{step_7.index}.produce')
    step_8.add_argument(name='reference', argument_type=ArgumentType.CONTAINER, data_reference=f'steps.{step_2.index}.produce')
    step_8.add_output('produce')
    step_8.add_hyperparameter(name = 'use_columns', argument_type=ArgumentType.VALUE, data = [0,1])
    pipeline_description.add_step(step_8)

    # Final Output
    pipeline_description.add_output(name='output predictions', data_reference=f'steps.{step_8.index}.produce')

    return pipeline_description


def test_augmentation(config, augmentation_method="channel_shuffle"):
    """Build a pipline based on the config
    """
    from d3m import index
    from d3m.metadata.base import ArgumentType
    ...
    # Step 6: Video Augmentation
    alg_python_path = 'd3m.primitives.autovideo.augmentation.' + augmentation_method
    step_6 = PrimitiveStep(primitive=index.get_primitive(alg_python_path))
    step_6.add_argument(name='inputs', argument_type=ArgumentType.CONTAINER, data_reference=f'steps.{step_5.index}.produce')
    #for key, value in config.items():
    #    step_6.add_hyperparameter(name=key, argument_type=ArgumentType.VALUE, data=value)
