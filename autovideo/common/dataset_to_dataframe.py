...
class DatasetToDataFramePrimitive(transformer.TransformerPrimitiveBase[Inputs, Outputs, Hyperparams]):
    ...
    metadata = metadata_base.PrimitiveMetadata(
        {
            ...
            'python_path': 'd3m.primitives.autovideo.common.dataset_to_dataframe',
            'source': {
    ...
    def produce(self, *, inputs: Inputs, timeout: float = None, iterations: int = None) -> base.CallResult[Outputs]:
        dataframe_resource_id, dataframe = base_utils.get_tabular_resource(inputs, self.hyperparams['dataframe_resource']) #Converts to dataframe
